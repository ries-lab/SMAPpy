...
# Upstream path: comet/core/drift_optimizer.py
#
...
# The cost-function fast paths (``cpu_wrapper_chunked_fast``,
# ``cpu_wrapper_chunked_approx``) are additions for smappy's drift
# correction; everything upstream provides behaves as it does upstream.
# ``io_utils`` and ``_dialogs`` are imported inside the functions that use
# them rather than at module level, so pandas and tkinter are needed only
# if those saving paths are actually taken.
...
from .qc_utils import flag_flawed_segments, plot_q_with_baseline

try:
    from .pytorch_wrapper.pytorch_util import device_available
    from .pytorch_wrapper.pytorch_wrapper import torch_wrapper_chunked
    from .pytorch_wrapper.pytorch_wrapper_qc import torch_wrapper_chunked_qc
except ModuleNotFoundError:
    pass # torch not installed
from .segmenter import segmentation_wrapper
from .cpu_wrapper import cpu_wrapper_chunked, cpu_wrapper_chunked_qc
...
def _save_dataset_as_ms_h5(*args, **kwargs):
    from .io_utils import save_dataset_as_ms_h5
    return save_dataset_as_ms_h5(*args, **kwargs)


def _save_drift_correction_details(*args, **kwargs):
    from .io_utils import save_drift_correction_details
    return save_drift_correction_details(*args, **kwargs)
...
# ----------------------------------------------------------------------------
from .interpolation import interpolate_drift
import time
...
def comet_run_kd(dataset, segmentation_mode, segmentation_var, max_locs_per_segment=None,
    ...
    if save_corrected_locs:
        if save_filepath is None:
            save_filepath = _ask_save_filename(title="Save drift corrected localizations as molecule set",
                                              defaultextension='h5')
        ...
        _save_dataset_as_ms_h5(dataset[:, :3], dataset[:, -1], pixelsize_nm,
                              pixelsize_z_nm=pixelsize_z_nm, filename=save_filepath)

    if save_correction_details:
        if save_filepath is None:
            save_filepath = _ask_save_filename(title="Save drift correction details in h5 file",
                                              defaultextension='h5')
        else:
            save_filepath = save_filepath.replace(".h5", "_details.h5")

        _save_drift_correction_details(save_filepath, drift_est, drift_interp, frame_interp,
...
def save_intermediate_results_wrapper(drift_est_nm, locs_nm, sigma_nm, sigma_factor, itr_counter, fails,
    ...
    if filehandle is None:
        filename = _ask_save_filename(title="Save intermediate results h5 file",
                                     defaultextension=".h5")
